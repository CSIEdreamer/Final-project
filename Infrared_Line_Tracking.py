...
#			time.sleep(0.0001)
	...
	def readStopLine(self, white_line = 0):
		sensor_values = self.readCalibrated()
		sum = 0
		for i in range(0,self.numSensors):
			value = sensor_values[i]
			...
			if(value > 200):
				on_line = 1
			# only average in values that are above a noise threshold
			diff = abs(value - self.calibratedMin[i])
			sum += diff
		#print(sum)
		#sum = 4000 - sum
		if(sum < 700):
	...
	def readContinue(self, white_line = 0, left=1):
		sensor_values = self.readCalibrated()
		sum = 0
		avg = 0
		for i in range(0,self.numSensors):
...
if __name__ == '__main__':
	...
	while True:
		...
		// improve performance.
		'''
		power_difference = proportional/25 + derivative/1000 #+ integral/1000;  
